...
pub fn menu_button<'a, R>(
  ...
  stationary_menu_impl(ui, button, Box::new(add_contents))
}
...
///
/// Responds to primary clicks.
fn stationary_menu_impl<'c, 'a, R>(
  ui: &mut Ui,
  mut button: Button<'a>,
  add_contents: Box<dyn FnOnce(&mut Ui) -> R + 'c>,
) -> InnerResponse<Option<R>> {
  let bar_id = ui.id();
  let menu_id = bar_id.with("menu");
...
impl MenuRoot {
  ...
  fn stationary_interaction(button: &Response, root: &mut MenuRootManager) -> MenuResponse {
    ...
    } else if (button.clicked() && !root.is_menu_open(id)) || (button.hovered() && root.is_some()) {
